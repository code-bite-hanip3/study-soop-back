...
const STUDIES = [
  // 이미지 카드 1~6 (첫 바퀴)
  {
    ...
    backgroundValue: '/images/banff.png',
    reactions: [
      ['👤', 37],
      ['🔥', 26],
      ['❤️', 14],
  ...
  },
  {
    ...
    points: 230,
    backgroundType: 'IMAGE',
    backgroundValue: '/images/calgary.png',
    reactions: [
      ['🚩', 21],
      ['🔥', 33],
      ['👤', 18],
  ...
  },
  {
    ...
    points: 145,
    backgroundType: 'IMAGE',
    backgroundValue: '/images/canmore.png',
    reactions: [
      ['❤️', 42],
      ['👍', 15],
      ['👤', 9],
  ...
  },
  {
    ...
    points: 375,
    backgroundType: 'IMAGE',
    backgroundValue: '/images/hanGang.png',
    reactions: [
      ['🔥', 51],
      ['👤', 28],
      ['❤️', 20],
  ...
  },
  {
    ...
    points: 198,
    backgroundType: 'IMAGE',
    backgroundValue: '/images/listBack.png',
    reactions: [
      ['💬', 24],
      ['❤️', 36],
      ['👍', 12],
  ...
  {
    name: '샌프란시스코 여행 일본어 회화',
    description: '여행지에서 바로 쓸 실전 표현을 몰아 익혀요',
    days: 7,
    points: 88,
    backgroundType: 'IMAGE',
    backgroundValue: '/images/sf.png',
    reactions: [
      ['👤', 13],
      ['👍', 19],
      ['🚩', 7],
  ...
  // 파스텔 카드 19~30 (green/yellow/blue 3색 순환 → blue 4장 추가)
  {
    ...
    backgroundValue: PASTEL_COLORS.green,
    reactions: [
      ['🔥', 19],
      ['👤', 22],
      ['❤️', 12],
  ...
  {
    name: '아침 30분 영어 회화 스파링',
    description: '커피 한 잔과 함께 오늘의 주제를 말해봐요',
    days: 11,
    points: 98,
    backgroundType: 'COLOR',
    backgroundValue: PASTEL_COLORS.yellow,
    reactions: [
      ['👤', 15],
      ['👍', 18],
      ['🔥', 6],
  ...
  },
  {
    ...
    points: 251,
    backgroundType: 'COLOR',
    backgroundValue: PASTEL_COLORS.blue,
    reactions: [
      ['🔥', 45],
      ['❤️', 24],
      ['💬', 13],
...
const seed = async (prisma) => {
  ...
  const COMPLETED_DURATION_MINUTES = [
    25, 40, 60, 90, 10, 50, 30, 75, 20, 100, 45, 15, 35, 55, 80, 42, 70, 65, 48,
    28, 58, 88, 95, 52, 24, 38, 66, 74, 12, 33,
  ];
  const focusSessionData = [];
  for (const study of studies) {
    const sessionCount = faker.number.int({ min: 5, max: 23 });

    for (let i = 0; i < sessionCount; i++) {
      const minutes = faker.helpers.arrayElement(COMPLETED_DURATION_MINUTES);
      ...
      focusSessionData.push({
        ...
        earnedPoint:
          Math.floor(minutes / 10) * POINTS_PER_10_MINUTES + BASE_POINTS,
  ...
  const pointHistoryData = studies.map((study) => {
    const sessionsForThisStudy = focusSessions.filter(
      (session) => session.studyId === study.id,
    );

    const totalAmount = sessionsForThisStudy.reduce(
      (sum, s) => sum + s.earnedPoint,
      0,
    );

    const latestSession = sessionsForThisStudy.reduce((latest, s) =>
      s.createdAt > latest.createdAt ? s : latest,
    );

    return {
      studyId: study.id,
      focusSessionId: latestSession.id,
      amount: totalAmount,
    };
  });
